...
export async function generateFollowUpReport(eventId, { preview = false } = {}) {
  try {
    const response = await api.get(`/staff/reports/follow-up/${eventId}`, {
      params: { format: 'pdf', ...(preview ? { preview: 1 } : {}) },
      responseType: 'blob',
    });
    ...
    if (preview) {
      ...
      link.download = `follow-up-${eventId}.pdf`;
    }
